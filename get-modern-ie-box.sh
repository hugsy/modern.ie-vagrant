...
if [ ! -f MSEdge-Win10.box ]; then
  ...
  while [ -z "$box_url" ]; do
    ...
    box_url=$(google-chrome-stable --headless --disable-gpu --dump-dom https://developer.microsoft.com/en-us/microsoft-edge/tools/vms/ | tr -d '\n' | egrep -o "http[^\'\"]*VirtualBox.zip")
  done
